...
类型：WS2812B 数字 RGB LED，作为 Core V1 主显示。

连接：

- `+5V` -> D3 1N4148WS -> `LED5V`
- `LED5V` -> D1 VDD
- D1 GND -> GND
- GPIO4 -> R4 330 ohm -> D1 DIN
- D1 DOUT -> P1 DATA 扩展焊盘
- D1 VDD/GND 就近放 C7 100 nF

说明：

- WS2812B 在 5V 供电时输入高电平门槛约为 0.7 x VDD，ESP32-C3 的 3.3V GPIO 会踩线。
- D3 把 D1 供电降到约 4.3V，使门槛降到约 3.0V，3.3V 数据高电平有余量。
- 外接灯带焊盘 P1 仍给满 5V，板载 D1 是链上第一颗。
...
- `+5V` -> WS2812_5V 焊盘（给外接灯带满 5V）
- `GND` -> WS2812_GND 焊盘
- D1 DOUT -> WS2812_DIN 焊盘

说明：

- V1 默认不装外接灯环。
- 外接灯环/灯带时，按“板载 D1 + 外接灯数”的总数配置固件 `NUM_LEDS`。
