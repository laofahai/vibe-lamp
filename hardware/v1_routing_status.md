...
| 短路 | **0** ✅ |
| 未连接 | **0** ✅ |
| DRC 违规 | **19**（全部非阻断:10 个 USB-C 连接器固有 + 9 个 cosmetic 丝印） |
| BOM 关键料号 | **7/7 已填**(U1/U2/J1/D2/D1/SW1/D3),被动件 JLCPCB 自动匹配 |
...
- 新增 **C7 100nF** 给 D1 就近去耦(接 LED5V)。
- **电平保障 D3 = 1N4148WS(C2128,基础库)**:WS2812B 数据高电平门槛=0.7×VDD,5V 供电时为 3.5V > C3 的 3.3V GPIO,会"踩线"。串一颗 D3 把 D1 供电从 5V 降到 ~4.3V(新网络 `LED5V`),门槛降到 ~3.0V,3.3V 数据**稳达标**。外接灯带口 P1 仍接满 5V。**全 PCBA、无需手焊**。
